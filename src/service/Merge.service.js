...
let createUser = (email) => {
    return new Promise((resolve, reject)=>{
        ...
        .headers({'Accept': 'application/json', 'Content-Type': 'application/json', pj: '586bb85baa5bdf0644e494da', app: 'facebook|google'})
        .send({
            username: email,
            status: 1,
            roles: ['586bb85baa5bdf0644e494db']
        })
        .end((resp) => {
            switch (resp.code) {
                case 200:                    
...
exports = module.exports = async () => {
    ...
    try{        
        let rs =await dbo.find({distinct: true, where: 'email'}, db.FAIL);
        for(let email of rs){        
            if(!email) continue;
