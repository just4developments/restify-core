...
global.GenType = {
        Uuid(mode='db', defaultValue) {
            ...
            Uuid.gen = (fieldName) => {
                ...
                Uuid.validateDeService = (item) => {
                    if(Uuid.mode === 'code') return `${item}.${fieldName} = utils.valid('${fieldName}', ${item}, ${Uuid.type});`;
                    else if(Uuid.mode === 'db') return `${item}.${fieldName} = db.uuid(utils.valid('${fieldName}', ${item}, [${Uuid.type}, db.Uuid]));`;
                }
                Uuid.validateInService = Uuid.validateUpService = (item) => {
                    ...
                    else if(Uuid.mode === 'db') return `${item}.${fieldName} = db.uuid(utils.valid('${fieldName}', ${item}.${fieldName}, [${Uuid.type}, db.Uuid]));`;
                }
                Uuid.validateFiService = (item) => {
                    return null;
                };
                Uuid.validateGeService = (item) => {
                    if(Uuid.mode === 'code') return `${item}.${fieldName} = utils.valid('${fieldName}', ${item}, ${Uuid.type});`;
                    else if(Uuid.mode === 'db') return `${item}.${fieldName} = db.uuid(utils.valid('${fieldName}', ${item}, [${Uuid.type}, db.Uuid]));`;
                }
                Uuid.assignInController = Uuid.assignUpController = `body.${fieldName} = ${Uuid.sign}req.body.${fieldName};`;
                ...
                if (defaultValue === undefined) {
                    ...
                    Uuid.assignInController = `body.${fieldName} = utils.uuid();`;
                }                                
            }            
            return Uuid;
        },
        Key(type = GenType.Uuid, defaultValue) {   
            ...
            Key.gen = (fieldName) => {
                ...
                Key.validateInService = (item) => {
                    if(Key.mode === 'code') return `${item}.${fieldName} = utils.uuid();`;
                    else if(Key.mode === 'db') return `${item}.${fieldName} = db.uuid();`;
                }
                Key.validateDeService = (item) => {
                    if(Key.mode === 'code') return `${item}.${fieldName} = utils.valid('${fieldName}', ${item}, ${Key.type});`;
                    else if(Key.mode === 'db') return `${item}.${fieldName} = db.uuid(utils.valid('${fieldName}', ${item}, [${Uuid.type}, db.Uuid]));`;
                }
                Key.validateUpService = (item) => {
                    if(Key.mode === 'code') return `${item}.${fieldName} = utils.valid('${fieldName}', ${item}.${fieldName}, ${Key.type});`;
                    else if(Key.mode === 'db') return `${item}.${fieldName} = db.uuid(utils.valid('${fieldName}', ${item}.${fieldName}, [${Uuid.type}, db.Uuid]));`;
                }
