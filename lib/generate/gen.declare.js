        ...
        Key(type = GenType.Uuid, defaultValue) {   
            ...
            Key.gen = (fieldName) => {
                ...
                Key.validateDeService = Key.validateGeService = (item) => {
                    ...
                    else if(type.mode === 'db') return `${item} = db.uuid(utils.valid('${fieldName}', ${item}, [${type.type}, db.Uuid]));`;
                }
                Key.validateInService = Key.validateUpService = (item) => {
                    if(type.mode === 'code') return `${item}.${fieldName} = utils.valid('${fieldName}', ${item}.${fieldName}, ${type.type});`;
                    else if(type.mode === 'db') return `${item}.${fieldName} = db.uuid(utils.valid('${fieldName}', ${item}.${fieldName}, [${type.type}, db.Uuid]));`;
                }
        ...
        Array(schema, defaultValue) {
            ...
            Array.gen = (fieldName) => {
                ...
                if (defaultValue === undefined) {
                    ...
                    const validate = (item, type, i) => {
                        let cnt = `${item}.${fieldName} = utils.valid('${fieldName}', ${item}.${fieldName}, Array, ${JSON.ostringify(defaultValue, null, '\t')});`;
