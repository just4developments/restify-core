...
global.GenType = {
        Uuid(mode='db', defaultValue) {
            ...
            Uuid.mode = mode;
            Uuid._gen = (fieldName, Uuid) => {
                Uuid.fieldName = fieldName;
                if (defaultValue !== null) {
                    Uuid.validateInService = (item) => {
                        if(Uuid.name === 'Key'){
                            if(mode === 'code') return `${item}.${fieldName} = utils.uuid();`;
                            else if(mode === 'db') return `${item}.${fieldName} = db.uuid();`;
                        }else {
                            if(mode === 'code') return `${item}.${fieldName} = utils.valid('${fieldName}', ${item}, [String, utils.Uuid]);`;
                            else if(mode === 'db') return `${item}.${fieldName} = db.uuid(utils.valid('${fieldName}', ${item}, [String, db.Uuid]));`;
                        }                        
                    }
                    ...
                    Uuid.validateUpService = (item) => {
                        if(mode === 'code') return `${item}.${fieldName} = utils.valid('${fieldName}', ${item}.${fieldName}, [String, utils.Uuid]);`;
                        else if(mode === 'db') return `${item}.${fieldName} = db.uuid(utils.valid('${fieldName}', ${item}.${fieldName}, [String, db.Uuid]));`;
                    }
                    ...
                    if(Uuid.name === 'Key'){
                        Uuid.assignUpController = `body.${fieldName} = req.params.${fieldName};`;
                    }else {
                        Uuid.assignUpController = Uuid.assignInController = `if(utils.has(req.body.${fieldName}) === true) body.${fieldName} = req.body.${fieldName};`;
                    }
                } 
