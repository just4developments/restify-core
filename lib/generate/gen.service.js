            ...
            try {
                ...
                db.open(exports.COLLECTION).then((db) => {
                    db.get(item.${fieldsKeyType.fieldName}, db.FAIL).then((oldItem) => {
                        ${assignFiles}
                        db.update(item, db.DONE).then((rs) => {                            
                            ${deleteOldFiles}
                            resolve(rs);
                        }).catch(errorHandle);
                    }).catch(errorHandle);
            ...
            try {
                ...
                db.open(exports.COLLECTION).then((db) => {
                    db.get(item.${fieldsKeyType.fieldName}, db.FAIL).then((item) => {
                        db.update(item, db.DONE).then(resolve).catch(reject);
                    }).catch(reject);
                }).catch(reject);
            } catch (e) {
                reject(e);
    ...
    this.delete = (fieldsKeyType, fieldsType) => {
            ...
            try {
                ...
                db.open(exports.COLLECTION).then((db) => {
                    db.get(${fieldsKeyType.fieldName}, db.FAIL).then((item) => {                    
                        db.delete(${fieldsKeyType.fieldName}, db.DONE).then((rs) => {
                            ${deleteFiles}
                            resolve(rs);
                        }).catch(reject);                        
                    }).catch(reject);
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                db.open(exports.COLLECTION).then((db) => {
                    db.get(${fieldsKeyType.fieldName}, db.FAIL).then((item) => {                    
                        db.delete(${fieldsKeyType.fieldName}, db.DONE).then(resolve).catch(reject);                        
                    }).catch(reject);
                }).catch(reject);
            } catch (e) {
                reject(e);
