...
server.get('/${tbl}', utils.jsonHandler(), (req, res, next) => {
    return ${tbl}Service.find({}).then((rs) => {
        res.send(rs);
    }).catch(next);
});

server.get('/${tbl}/:${key}', utils.jsonHandler(), (req, res, next) => {
...
server.del('/${tbl}/:${key}', utils.jsonHandler(), (req, res, next) => {
