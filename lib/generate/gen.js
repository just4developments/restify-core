...
let generate = (tbl, col) => {
    ...
    for (var colName in col) {
        if (col[colName].prototype.constructor.name === 'Key') {
            ...
            assignIValue.push(`if(utils.has(req.body.${colName}, ${typeName})) body.${colName} = req.body.${colName};`);
            assignUValue.push(`if(utils.has(req.body.${colName}, ${typeName})) body.${colName} = req.body.${colName};`);       
        } else if(col[colName].prototype.constructor.name === 'FileUpload') {
            jsonBodyParser = col[colName].prototype.config;            
            assignIValue.push(`if(utils.has(req.file.${colName})) body.${colName} = req.file.${colName};`);
            assignUValue.push(`if(utils.has(req.file.${colName})) body.${colName} = req.file.${colName};`);
            ...
            if(col[colName].prototype.ignore && ['Dateignore'].indexOf(col[colName].prototype.constructor.name) != -1){
                ...
                assignIValue.push(`if(utils.has(req.body.${colName})) body.${colName} = ${(['Number', 'Numberable'].indexOf(col[colName].prototype.constructor.name) !== -1) ? '+' : ''}req.body.${colName};`);
                assignUValue.push(`if(utils.has(req.body.${colName})) body.${colName} = ${(['Number', 'Numberable'].indexOf(col[colName].prototype.constructor.name) !== -1) ? '+' : ''}req.body.${colName};`);
            }
        }
    ...
    // Controller
    let fcontroller = path.join(__dirname, '..', '..', config.outdir, 'controller', `${tbl}.controller.js`);
    try {
    ...
    // Http testing
    let fhttp = path.join(__dirname, '..', '..', config.outdir, 'test', `${tbl}.http.txt`);
    try {
        fs.statSync(fhttp);
