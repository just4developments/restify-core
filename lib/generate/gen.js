...
global.GenType = {
        Key: (type, defaultValue) => {
            ...
            Key.gen = (fieldName) => {
                ...
                Key.validateDeService = `if(!utils.has(item)) throw new restify.BadRequestError('${fieldName} is required!');`;
                Key.validateUpService = `if(!utils.has(item.${fieldName})) throw new restify.BadRequestError('${fieldName} is required!');`;
        ...
        String: (defaultValue) => {
            ...
            String.gen = (fieldName) => {
                ...
                if (defaultValue === undefined) {
                    ...
                    String.validateUpService = String.validateInService = `if(!utils.has(item.${fieldName})) item.${fieldName} = ${JSON.stringify(defaultValue)};`;
                }
        ...
        Number: (defaultValue) => {
            ...
            Number.gen = (fieldName) => {
                ...
                if (defaultValue === undefined) {
                    ...
                    // Number.validateUpService = Number.validateInService = `item.${fieldName} = +item.${fieldName} || null;`;                    
                } else {
                    Number.upHttp = Number.inHttp = `${fieldName}: ${JSON.stringify(defaultValue)}`;
                    Number.validateUpService = Number.validateInService = `if(!utils.has(item.${fieldName})) item.${fieldName} = ${JSON.stringify(defaultValue)};`;
                }
        ...
        Date: (...yyyy) => {
            ...
            Date.gen = (fieldName) => {
                ...
                if (defaultValue === undefined) {
                    ...
                    Date.validateUpService = Date.validateInService = `if(!utils.has(item.${fieldName})) item.${fieldName} = ${defaultValue};`;
                }
        ...
        Object: (defaultValue) => {
            ...
            Object.gen = (fieldName) => {
                ...
                Object.assignUpController = Object.assignInController = `body.${fieldName} = utils.object(req.body.${fieldName});`;
                if (defaultValue === undefined) {
...
if(!utils.is(item.${fieldName}, Object)) throw new restify.BadRequestError('${fieldName} must be Object!');`;
                    ...
                    Object.validateUpService = Object.validateInService = `if(!utils.has(item.${fieldName})) item.${fieldName} = ${JSON.stringify(defaultValue)};`;
                }
        ...
        Array: (defaultValue) => {
            ...
            Array.gen = (fieldName) => {
                ...
                Array.assignUpController = Array.assignInController = `body.${fieldName} = utils.object(req.body.${fieldName});`;
                if (defaultValue === undefined) {
...
if(!utils.is(item.${fieldName}, Array)) throw new restify.BadRequestError('${fieldName} must be Array!');`;
                    ...
                    Array.validateUpService = Array.validateInService = `if(!utils.has(item.${fieldName})) item.${fieldName} = ${JSON.stringify(defaultValue)};`;
        ...
        File: (config, defaultValue) => {
                ...
                File.gen = (fieldName) => {
                    ...
                    if (defaultValue === undefined) {                        
                        ...
                        // File.validateUpService = File.validateInService = `item.${fieldName} = item.${fieldName} || null;`;
                        File.upHttp = File.inHttp = `'file:${fieldName}': null`;
                    } else {
                        File.validateUpService = File.validateInService = `if(!utils.has(item.${fieldName})) item.${fieldName} = ${JSON.stringify(defaultValue)};`;
