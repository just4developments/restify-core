...
let generate = (tbl, col) => {
    ...
    let catchAfterUploadError = `next`;
    let ivalidation = [];
    let uvalidation = [];
    ...
    for (var colName in col) {
        if (col[colName].prototype.constructor.name === 'Key') {
            ...
            assignIValue.push(`if(req.files.${colName}) body.${colName} = utils.getPathUpload(req.files.${colName}, '${col[colName].prototype.returnPath}', ${col[colName].prototype._config.multiples ? 'true' : 'false'});`);
            assignUValue.push(`if(req.files.${colName}) body.${colName} = utils.getPathUpload(req.files.${colName}, '${col[colName].prototype.returnPath}', ${col[colName].prototype._config.multiples ? 'true' : 'false'});`);
            catchAfterUploadError = `(err) => {
        utils.deleteFile(utils.getAbsoluteUpload(body.${colName}));
        next(err);
    }`;
    ...
    if(removeFileWhenDelete.length === 0){
        removeFileWhenDelete.push(
                `db().open().then((db) => {
                    db.delete(${key}).then(resolve).catch(reject0);
                }).catch(reject0)`);
    }
    // Controller
    genHelper.export(path.join(__dirname, '[name].controller.js'), {
        ...
        assignIValue: assignIValue,
        assignUValue: assignUValue,
        catchAfterUploadError: catchAfterUploadError,
