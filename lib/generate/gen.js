...
let generate = (tbl, col) => {
    ...
    for (var colName in col) {
        if (col[colName].prototype.constructor.name === 'Key') {
            ...
            removeFileWhenIUError = `utils.deleteFile(utils.getAbsoluteUpload(obj.${colName}, path.join(__dirname, '..', '..', '${col[colName].prototype._config.uploadDir.split('/').join("', '")}')), ${global.Native.toString(col[colName].prototype._config.resize)});`;            
